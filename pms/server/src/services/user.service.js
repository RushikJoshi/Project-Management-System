...
export async function createUser({ companyId, workspaceId, actorRole, input }) {
  ...
  if (input.role && ROLE_HIERARCHY[actorRole] < ROLE_HIERARCHY[input.role]) {
...
export async function updateUser({ companyId, workspaceId, actorRole, userId, targetUserId, updates }) {
  ...
  // RBAC: Actor cannot manage someone with a higher role level
  if (ROLE_HIERARCHY[actorRole] < ROLE_HIERARCHY[user.role]) {
    const err = new Error('You do not have permission to manage this user');
    err.statusCode = 403;
    err.code = 'FORBIDDEN_TARGET_ROLE';
    throw err;
  }
  ...
  // RBAC: Actor can only assign roles at or below their own level
  if (updates.role && ROLE_HIERARCHY[actorRole] < ROLE_HIERARCHY[updates.role]) {
