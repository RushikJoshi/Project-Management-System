  ...
  try {
    ...
    allowedUpdates.forEach(key => {
      if (req.body[key] !== undefined) {
        ...
        if (key === 'goals') value = (value || []).filter(g => g.target?.trim());
        if (key === 'learnings') value = (value || []).filter(l => l.challenge?.trim() || l.lesson?.trim());
        if (key === 'keyTasks') value = (value || []).filter(k => k.task?.trim());
        mis[key] = value;
      }
    });
