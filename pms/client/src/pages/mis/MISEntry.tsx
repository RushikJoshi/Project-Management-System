...
export default function MISEntry() {
  ...
  const handleSave = async (submit: boolean = false) => {
    ...
    try {
      const filteredGoals = goals.filter(g => g.target.trim() !== '');
      const filteredLearnings = learnings.filter(l => l.challenge.trim() !== '' || l.lesson.trim() !== '');
      const filteredKeyTasks = keyTasks.filter(k => k.task.trim() !== '');

      if (submit) {
        if (filteredGoals.length === 0) { alert('Please add at least one goal.'); return; }
        if (filteredLearnings.length === 0) { alert('Please add at least one learning.'); return; }
        if (filteredKeyTasks.length === 0) { alert('Please add at least one key task.'); return; }

        for (const g of filteredGoals) {
          if (!g.target.trim()) { alert('Goal target is required for all goal entries.'); return; }
        }
        for (const l of filteredLearnings) {
          if (!l.challenge.trim() || !l.lesson.trim()) { alert('Both challenge and lesson are required for all learning entries.'); return; }
        }
        for (const k of filteredKeyTasks) {
          if (!k.task.trim()) { alert('Task description is required for all key task entries.'); return; }
        }
      }
