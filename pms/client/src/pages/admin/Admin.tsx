...
export const AdminUsersPage: React.FC = () => {
  ...
  const filtered = users.filter(u => {
    // RBAC: Only show users at or below current user's role level
    if (user && ROLE_HIERARCHY[user.role] < ROLE_HIERARCHY[u.role]) return false;
  ...
  const ROLES: { value: Role | 'all'; label: string }[] = [
    ...
    ...Object.entries(ROLE_CONFIG)
      .filter(([role]) => !user || ROLE_HIERARCHY[user.role] >= ROLE_HIERARCHY[role as Role])
      .map(([role, cfg]) => ({ value: role as Role, label: cfg.label }))
  ];

  const CREATE_ROLE_OPTIONS = Object.keys(ROLE_CONFIG)
    .filter((role) => !user || ROLE_HIERARCHY[user.role] >= ROLE_HIERARCHY[role as Role]) as Role[];
  ...
  const handleSaveUser = async () => {
    ...
    try {
      ...
      // Prevent sending role if user is editing themselves to avoid backend restriction
