...
export const KanbanBoard = forwardRef<KanbanBoardHandle, KanbanBoardProps>(({
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    const effectiveOverId = overId.startsWith('column:') ? overId.replace('column:', '') : overId;

    const targetColumnId = orderedColumns.some((column) => column.id === effectiveOverId)
      ? effectiveOverId
      : (() => {
          const targetTask = projectTasks.find((task) => task.id === effectiveOverId);
