...
const CreateProjectPage: React.FC = () => {
  ...
  const { bootstrap, users, teams } = useAppStore();
  ...
  const [selectedLeaderId, setSelectedLeaderId] = useState<string>(currentUser?.id || '');
  ...
  const [selectedReporters, setSelectedReporters] = useState<string[]>(() => {
    ...
    return admin ? [admin.id] : (currentUser?.id ? [currentUser.id] : []);
  });
    ...
    <div className="bg-white dark:bg-surface-950 flex flex-col font-sans -mt-6">
      <div className="flex items-center justify-between pb-3 border-b border-surface-50 dark:border-surface-800/50 mb-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={handleSubmit(onSubmit)} disabled={!isValid || isSubmitting || !selectedLeaderId} className={cn("px-8 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all", (isValid && selectedLeaderId) ? "bg-brand-600 text-white shadow-lg shadow-brand-500/10" : "bg-surface-100 text-surface-400 cursor-not-allowed")}>{isSubmitting ? 'Creating...' : 'Create Project'}</button>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div className="grid grid-cols-12 gap-6 pt-1">
          <div className="col-span-12 lg:col-span-4 space-y-1.5">
            <label className="text-[9px] font-black uppercase tracking-[0.2em] text-surface-300 flex items-center gap-2"><Users size={10} className="text-brand-500" /> Project Lead *</label>
            <div ref={leaderRef} className="relative">
              <div onClick={() => setShowLeaderDrop(true)} className="h-9 bg-surface-50/50 dark:bg-surface-900/30 border border-surface-100 dark:border-surface-800 rounded-xl px-3 flex items-center justify-between cursor-pointer">
                {selectedLeaderId ? (
                  <div className="flex items-center gap-2">
                    <UserAvatar name={users.find(u => u.id === selectedLeaderId)?.name || ''} avatar={users.find(u => u.id === selectedLeaderId)?.avatar} size="xs" />
                    <span className="text-[11px] font-bold text-surface-700 dark:text-surface-200">{users.find(u => u.id === selectedLeaderId)?.name}</span>
                  </div>
                ) : (
                  <span className="text-surface-400 text-[10px]">Select lead...</span>
                )}
              ...
              <AnimatePresence>
                {showLeaderDrop && (
                  <motion.div initial={{ opacity: 0, y: -5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -5 }} className="absolute z-50 left-0 right-0 mt-1 bg-white dark:bg-surface-900 border border-surface-100 dark:border-surface-800 rounded-xl shadow-xl overflow-hidden">
                    <div className="p-2 border-b border-surface-50 dark:border-surface-800"><input autoFocus value={leaderQuery} onChange={e => setLeaderQuery(e.target.value)} placeholder="Search leader..." className="w-full bg-surface-50 dark:bg-surface-800 rounded-lg px-3 py-1.5 text-[11px] outline-none" /></div>
                    <div className="max-h-40 overflow-y-auto py-1">
                      {filteredUsers(leaderQuery).map(u => (
                        <div key={u.id} onClick={() => { setSelectedLeaderId(u.id); setShowLeaderDrop(false); }} className={cn("px-3 py-1.5 flex items-center gap-2 cursor-pointer hover:bg-surface-50 dark:hover:bg-surface-800", selectedLeaderId === u.id && "bg-brand-50 dark:bg-brand-950/20")}>
                          ...
                          {selectedLeaderId === u.id && <Check size={12} className="text-brand-500 ml-auto" />}
                        </div>
                      ))}
                    </div>
                  </motion.div>
          ...
          <div className="col-span-12 lg:col-span-8 space-y-1.5">
            <label className="text-[9px] font-black uppercase tracking-[0.2em] text-surface-300 flex items-center gap-2"><Users size={10} className="text-brand-500" /> Assign Team Members</label>
            <div ref={memberRef} className="relative">
              <div onClick={() => setShowMemberDrop(true)} className="min-h-[36px] bg-surface-50/50 dark:bg-surface-900/30 border border-surface-100 dark:border-surface-800 rounded-xl p-1 flex flex-wrap gap-1 cursor-pointer">
              ...
              <AnimatePresence>
                {showMemberDrop && (
                  <motion.div initial={{ opacity: 0, y: -5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -5 }} className="absolute z-50 left-0 right-0 mt-1 bg-white dark:bg-surface-900 border border-surface-100 dark:border-surface-800 rounded-xl shadow-xl overflow-hidden">
                    <div className="p-2 border-b border-surface-50 dark:border-surface-800"><input autoFocus value={memberQuery} onChange={e => setMemberQuery(e.target.value)} placeholder="Search members..." className="w-full bg-surface-50 dark:bg-surface-800 rounded-lg px-3 py-1.5 text-[11px] outline-none" /></div>
                    <div className="max-h-40 overflow-y-auto py-1">
        ...
        <div className="grid grid-cols-12 gap-6">
          ...
          <div className="col-span-12 lg:col-span-3 space-y-1.5">
            <label className="text-[9px] font-black uppercase tracking-[0.2em] text-surface-300 flex items-center gap-2"><Users size={10} className="text-brand-500" /> Assign Team (Optional)</label>
            <div ref={teamRef} className="relative">
              <button type="button" onClick={() => setIsTeamOpen(!isTeamOpen)} className="w-full h-9 bg-surface-50/50 dark:bg-surface-900/30 border border-surface-100 dark:border-surface-800 rounded-xl px-4 flex items-center justify-between transition-all outline-none leading-none">
                <span className="text-[11px] font-bold text-surface-700 dark:text-surface-200 truncate pr-2">
                  {teams.find(t => t.id === watch('teamId'))?.name || 'No Team Assigned'}
                </span>
                <ChevronDown size={14} className={cn("text-surface-300 transition-transform shrink-0", isTeamOpen && "rotate-180")} />
              </button>
              <AnimatePresence>
                {isTeamOpen && (
                  <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} className="absolute z-50 left-0 right-0 mt-1.5 py-1 bg-white dark:bg-surface-900 border border-surface-100 dark:border-surface-800 rounded-xl shadow-xl max-h-48 overflow-y-auto">
                    <button type="button" onClick={() => { setValue('teamId', ''); setIsTeamOpen(false); }} className={cn("w-full px-4 py-2 text-[11px] font-bold text-left hover:bg-surface-50 dark:hover:bg-surface-800 leading-none", !watch('teamId') ? "text-brand-600" : "text-surface-600 dark:text-surface-200")}>No Team</button>
                    {teams.map(team => (
                      <button key={team.id} type="button" onClick={() => { setValue('teamId', team.id); setIsTeamOpen(false); }} className={cn("w-full px-4 py-2 text-[11px] font-bold text-left hover:bg-surface-50 dark:hover:bg-surface-800 leading-none truncate", watch('teamId') === team.id ? "text-brand-600" : "text-surface-600 dark:text-surface-200")}>
                        {team.name}
                      </button>
                    ))}
                  </motion.div>
          ...
          <div className="col-span-12 lg:col-span-3 space-y-1.5 text-nowrap">
            <label className="text-[9px] font-black uppercase tracking-[0.2em] text-surface-300 flex items-center gap-2"><IndianRupee size={10} className="text-brand-500" /> Budget</label>
