...
import {
  ...
  ArrowLeft, Edit3, Calendar, Flag, Users, ChevronDown, Clock, Activity, Zap
} from 'lucide-react';
...
import type { Task, TaskStatus, TimelinePhase, TaskCreationRequest, Priority, ProjectSdlcPhase, Role } from '../../app/types';
...
export const ProjectDetailPage: React.FC = () => {
  ...
  const projectTasks = tasks.filter(t => {
    ...
    const matchesProject = String(pid) === String(id);
    ...
    return true; // Admin, Manager, Team Leader see all tasks in the project
  });
  const selectedTask = tasks.find(t => t.id === selectedTaskId) || null;
  const members = users.filter(u => project?.members.includes(u.id) && !['super_admin', 'admin'].includes(u.role));
  const isClient = user?.userType === 'client';
  const canCreateTask = !isClient && (user?.role !== 'team_member' || (project?.reportingPersonIds || []).includes(user?.id));
  const canEditProject = !isClient && (user?.role !== 'team_member' || canEditOtherProjects);
  const canRequestTask = Boolean(user?.id && project?.members.includes(user.id) && !canCreateTask);
  const canReviewTaskRequests = !isClient && Boolean(
    user?.id &&
    ((project?.reportingPersonIds || []).includes(user.id) || ['super_admin', 'admin', 'manager', 'team_leader'].includes(user?.role || ''))
  );
  const isAdmin = ['super_admin', 'admin', 'manager', 'team_leader', 'CLIENT_ADMIN', 'CLIENT_MANAGER'].includes(user?.role || '');
    ...
    <div className="max-w-full">
      ...
      <ProjectTaskCreateModal
        ...
        onUpdateProject={async (updates) => {
          try {
            const response = await projectsService.update(project.id, updates);
            updateProject(project.id, response.data.data ?? response.data);
            await bootstrap();
          } catch (error: any) {
            emitErrorToast('Failed to update project settings', 'Project');
          }
        }}
      />
