...
export default function MISEntry() {
  ...
  const handleSave = async (submit: boolean = false) => {
    // Basic Validation
    if (submit) {
      const emptyGoal = goals.some(g => !g.target.trim() || !g.actual.trim());
      const emptyLearning = learnings.some(l => !l.challenge.trim() || !l.lesson.trim());
      const emptyTask = keyTasks.some(t => !t.task.trim());

      if (emptyGoal) {
        setValidationError('Section 1: Business Goals are incomplete. Please ensure both "Goal/Target" and "Actual Work" are filled for all rows.');
        return;
      }
      if (emptyLearning) {
        setValidationError('Section 2: Learning From Last Week is incomplete. Please ensure both "Challenge Faced" and "Lesson Learned" are filled for all rows.');
        return;
      }
      if (emptyTask) {
        setValidationError('Section 3: Key Priority Tasks are incomplete. Please ensure the "Task/Focus Area" is filled for all rows.');
        return;
      }
    }

    if (submit && !showSubmitConfirm) {
    ...
    try {
      ...
      emitSuccessToast(submit ? 'MIS Submitted successfully!' : 'Draft saved!', submit ? 'Success' : 'Draft Saved');
      if (submit) setShowSubmitConfirm(false);
    } catch (err: any) {
      console.error(err);
      const msg = err.response?.data?.message || err.message || 'Unknown error';
      emitErrorToast(msg, 'Failed to save MIS');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="w-full font-sans pb-8">
      ...
      >
        <div className="p-6 text-center">
          <div className="w-16 h-16 bg-brand-50 dark:bg-brand-950/30 rounded-2xl flex items-center justify-center mx-auto mb-4 text-brand-600">
            <Send size={32} />
          </div>
          <h3 className="text-lg font-bold text-surface-900 dark:text-white mb-2">Ready to submit?</h3>
          <p className="text-sm text-surface-500 dark:text-surface-400 mb-8">
            Once submitted, you will not be able to edit this MIS entry. Please ensure all details are correct.
          </p>
          <div className="flex gap-3">
            <button
              onClick={() => setShowSubmitConfirm(false)}
              className="flex-1 px-4 py-2.5 rounded-xl border border-surface-200 dark:border-surface-700 text-surface-600 dark:text-surface-400 font-bold text-sm hover:bg-surface-50 dark:hover:bg-surface-800 transition-colors"
            >
              Cancel
            </button>
            <button
              onClick={() => handleSave(true)}
              disabled={loading}
              className="flex-1 px-4 py-2.5 rounded-xl bg-brand-600 hover:bg-brand-700 text-white font-bold text-sm shadow-lg shadow-brand-500/20 transition-all flex items-center justify-center gap-2"
            >
      ...
      >
        <div className="p-6 text-center">
          <div className="w-16 h-16 bg-rose-50 dark:bg-rose-950/30 rounded-2xl flex items-center justify-center mx-auto mb-4 text-rose-600">
            <AlertTriangle size={32} />
          </div>
          <h3 className="text-lg font-bold text-surface-900 dark:text-white mb-2">Incomplete Entry</h3>
          <p className="text-sm text-surface-500 dark:text-surface-400 mb-8 leading-relaxed">
            {validationError}
          </p>
          <button
            onClick={() => setValidationError(null)}
            className="w-full px-4 py-2.5 rounded-xl bg-surface-900 dark:bg-white text-white dark:text-surface-900 font-bold text-sm hover:opacity-90 transition-all shadow-lg"
          >
            I'll Fix It
          </button>
        </div>
