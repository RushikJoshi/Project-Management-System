...
const STATUS_CARDS: Array<{
  value: StatusFilter | ScopeFilter;
  label: string;
  tone: string;
  isScope?: boolean;
}> = [
    { value: 'all', label: 'All Tasks', tone: 'text-surface-900 dark:text-surface-100' },
    { value: 'assigned_to_me', label: 'Assigned to Me', tone: 'text-indigo-600 dark:text-indigo-300', isScope: true },
    { value: 'created_by_me', label: 'Created by Me', tone: 'text-purple-600 dark:text-purple-300', isScope: true },
...
export const QuickTasksPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (s && ['all', 'todo', 'in_progress', 'done', 'overdue'].includes(s)) {
      setStatus(s as StatusFilter);
    }
    
    const scopeParam = params.get('scope');
    if (scopeParam && ['all', 'assigned_to_me', 'created_by_me'].includes(scopeParam)) {
      setScope(scopeParam as ScopeFilter);
    }
  ...
  const counts = useMemo(() => {
    ...
    // For scope counts, we want to know the count after basic filters (search/dept/person) but BEFORE scope filter
    const afterOtherFilters = quickTasks.filter(t => {
      const reporter = userMap.get(t.reporterId);
      const assignees = (t.assigneeIds || []).map(id => userMap.get(id)).filter(Boolean);
      const depts = Array.from(new Set([reporter?.department?.trim() || '', ...assignees.map(a => a?.department?.trim() || '')].filter(Boolean)));
      
      if (departmentFilter !== 'all' && !depts.includes(departmentFilter)) return false;
      if (personFilter !== 'all' && t.reporterId !== personFilter && !(t.assigneeIds || []).includes(personFilter)) return false;
      
      const q = query.trim().toLowerCase();
      if (!q) return true;
      return [t.title, t.description || '', reporter?.name || ''].join(' ').toLowerCase().includes(q);
    });

    return {
      total: afterOtherFilters.length,
      assigned: afterOtherFilters.filter(t => (t.assigneeIds || []).includes(uid)).length,
      created: afterOtherFilters.filter(t => t.reporterId === uid).length,
  ...
  }, [quickTasks, baseFiltered, query, departmentFilter, personFilter, user?.id, userMap]);
  ...
  return (
    <div className="max-w-full mx-auto">
      <div className="grid grid-cols-1 gap-3 mb-4 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7">
        {STATUS_CARDS.map((card) => {
          const count =
            card.value === 'all' ? counts.total :
              card.value === 'assigned_to_me' ? counts.assigned :
                card.value === 'created_by_me' ? counts.created :
                  card.value === 'todo' ? counts.todo :
                    card.value === 'in_progress' ? counts.in_progress :
                      card.value === 'done' ? counts.done :
                        counts.overdue;

          const isActive = card.isScope 
            ? (status === 'all' && scope === card.value) 
            : (status === card.value && (card.value !== 'all' || scope === 'all'));
          ...
          return (
            <button
              ...
              onClick={() => {
                if (card.isScope) {
                  setScope(card.value as ScopeFilter);
                  setStatus('all');
                } else if (card.value === 'all') {
                  setScope('all');
                  setStatus('all');
                } else {
                  setStatus(card.value as StatusFilter);
                }
              }}
              className={cn(
