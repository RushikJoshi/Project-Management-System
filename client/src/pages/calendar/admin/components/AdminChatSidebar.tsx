...
export const AdminChatSidebar = () => {
    ...
    // Heartbeat — tell server this user is active every 30s
    useEffect(() => {
        if (!user) return;
        const sendHeartbeat = () => api.post('/admin/chat/heartbeat').catch(() => {});
        sendHeartbeat();
        const hbInterval = window.setInterval(sendHeartbeat, 30000);
        return () => window.clearInterval(hbInterval);
    }, [user]);

    // Fetch online users every 30s
    const fetchOnlineUsers = async () => {
        try {
            const res = await api.get('/admin/chat/online-users');
    ...
    useEffect(() => {
        fetchOnlineUsers();
        const onlineInterval = window.setInterval(fetchOnlineUsers, 30000);
    ...
    useEffect(() => {
        ...
        const interval = window.setInterval(() => {
            // Silent refresh — no loading spinner, no flicker
            fetchConversations(true);
            if (activeConversationId) {
                useAdminChatStore.getState().fetchMessages(activeConversationId);
            }
        }, 15000); // 15 seconds — reduce blinking
                                                            ...
                                                            <div className="divide-y divide-surface-50 dark:divide-surface-800/50">
                                                                {availableDirectUsers.map((item) => {
                                                                    const roleColor =
                                                                        ...
                                                                        item.role === 'team_leader' ? 'bg-amber-50 text-amber-600 dark:bg-amber-950/30 dark:text-amber-400 border-amber-200 dark:border-amber-800' :
                                                                        'bg-surface-50 text-surface-500 dark:bg-surface-800 dark:text-surface-400 border-surface-200 dark:border-surface-700';
                                                                    return (
                                                                        <button
                                                                        ...
                                                                        >
                                                                            <div className="relative w-9 h-9 shrink-0">
                                                                                <div className="w-9 h-9 rounded-full bg-gradient-to-br from-brand-100 to-brand-50 dark:from-brand-900/30 dark:to-brand-950/20 border border-brand-200 dark:border-brand-800 flex items-center justify-center text-xs font-bold text-brand-600 dark:text-brand-400">
                                     ...
                                     <div className="flex items-center gap-3 min-w-0">
                                         ...
                                         <div className="w-14 h-14 rounded-full bg-gradient-to-br from-surface-50 to-surface-100 dark:from-surface-800 dark:to-surface-900 border border-surface-200 dark:border-surface-700 flex items-center justify-center text-brand-600 dark:text-brand-400 relative shrink-0 shadow-inner">
                                             ...
                                             {!isGroupConversation(activeConvo) && activeConvo && (() => {
                                                const otherP = getOtherParticipant(activeConvo, user?.id || '');
                                                const isOtherOnline = otherP && onlineUserIds.has(participantId(otherP));
                                         ...
                                         <div className="flex-1 min-w-0">
                                              <div className="text-[15px] font-semibold text-surface-900 dark:text-white leading-tight truncate">
                                               ...
                                               <span className={cn(
                                                   ...
                                                   isGroupConversation(activeConvo) ? "text-surface-500 dark:text-surface-400" : (() => { const op = activeConvo ? getOtherParticipant(activeConvo, user?.id || "") : null; return op && onlineUserIds.has(participantId(op)) ? "text-emerald-500 dark:text-emerald-400" : "text-surface-400 dark:text-surface-500"; })()
                                               )}>
                                                   {isGroupConversation(activeConvo) ? `${activeConvo?.participants.length || 0} members` : (() => { const op = activeConvo ? getOtherParticipant(activeConvo, user?.id || '') : null; return op && onlineUserIds.has(participantId(op)) ? 'Online' : 'Offline'; })()}
                                               </span>
                                         </div>
                                     </div>
