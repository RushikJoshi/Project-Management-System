                                ...
                                <div className="relative mb-3 flex flex-col gap-3 rounded-[12px] bg-white dark:bg-surface-900 p-3 shadow-lg ring-1 ring-surface-200 dark:ring-surface-800">
                                    ...
                                    <div className="flex items-center justify-between pt-1">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <Dropdown
                                                value={inlineTag}
                                                onChange={setInlineTag}
                                                items={(Array.isArray(teams) ? teams : []).map(t => ({ id: t.name, label: t.name }))}
                                                placeholder="Team"
                                                triggerClassName="h-[26px] py-0 px-2 bg-surface-100 dark:bg-surface-800 rounded border-none text-[10px] font-black uppercase tracking-wider"
                                                icon={<ChevronDown size={14} strokeWidth={3} className="mr-0" />}
                                                hideChevron
                                            />
                                            ...
                                            <Dropdown
                                                value={inlineUser}
                                                onChange={setInlineUser}
                                                items={(Array.isArray(users) ? users : []).map(u => ({ id: u.name, label: u.name }))}
                                                triggerClassName="h-[26px] w-[26px] rounded-full bg-brand-500 text-white shadow-sm border-[1.5px] border-white dark:border-surface-800 p-0 flex items-center justify-center hover:brightness-95"
                                                icon={<User size={13} strokeWidth={3} />}
                                                hideLabel
                                                hideChevron
                                            />
                                            ...
                                            <Dropdown
                                                ...
                                                onChange={(val) => setInlinePriority(val as any)}
                                                items={[
                                                    { id: 'none', label: 'None', icon: <Flag size={12} strokeWidth={3} /> },
                                                    { id: 'high', label: 'High', icon: <Flag size={12} fill="currentColor" strokeWidth={3} className="text-red-500" /> },
                                                    { id: 'medium', label: 'Medium', icon: <Flag size={12} fill="currentColor" strokeWidth={3} className="text-amber-500" /> },
                                                    { id: 'low', label: 'Low', icon: <Flag size={12} fill="currentColor" strokeWidth={3} className="text-blue-500" /> },
                                                ]}
                                                triggerClassName={cn(
                                                    "h-[26px] flex items-center gap-1.5 rounded-full px-2 border-none transition-all py-0",
                                                    inlinePriority === 'high' ? "bg-red-50 dark:bg-red-950/30 text-red-600 dark:text-red-400" :
                                                    inlinePriority === 'medium' ? "bg-amber-50 dark:bg-amber-950/30 text-amber-600 dark:text-amber-400" :
                                                    inlinePriority === 'low' ? "bg-blue-50 dark:bg-blue-950/30 text-blue-600 dark:text-blue-400" :
                                                    "bg-surface-50 dark:bg-surface-800 text-surface-400 dark:text-surface-500"
                                                )}
                                                hideChevron
                                                icon={<Flag size={12} fill={inlinePriority !== 'none' ? 'currentColor' : 'none'} strokeWidth={3} />}
                                                placeholder="Prio"
                                            />
                                        </div>
                                        <button
