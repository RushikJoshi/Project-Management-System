...
export const TaskModal: React.FC<TaskModalProps> = ({ task, open, onClose, initialTab }) => {
  ...
  const [pendingSubtasksModalOpen, setPendingSubtasksModalOpen] = useState(false);
  ...
  const canManageTask = !isClient && ['super_admin', 'admin', 'manager', 'team_leader'].includes(user?.role || '');
  ...
  const canReview = !isClient && Boolean(
    user && (
      canManageTask ||
      (
  ...
  const pendingSubtasks = (localTask.subtasks ?? currentTask.subtasks ?? []).some(s => !s.isCompleted);
  ...
  const saveLocalChanges = async () => {
    ...
    try {
      ...
      if ((updates.status === 'done' || updates.status === 'in_review') && pendingSubtasks) {
        setPendingSubtasksModalOpen(true);
        return;
      }

      const response = await tasksService.update(currentTask.id, updates, { suppressErrorToast: true });
  ...
  const handleCompletionSubmit = async (remark: string, files: File[]) => {
    ...
    try {
      ...
      console.error('[handleCompletionSubmit] Error:', err);
      const code = err?.response?.data?.error?.code || err?.response?.data?.code;
      if (code === 'PENDING_SUBTASKS') {
        setPendingSubtasksModalOpen(true);
      } else {
  ...
  return (
    <Modal open={open} onClose={onClose} size="xl" showClose={false}>
      <div className="flex h-full max-h-[85vh] flex-col">
              ...
              <div className="space-y-5">
                ...
                <div>
                      ...
                      <input
                        ...
                        onKeyDown={(event) => {
                          if (event.key === 'Enter') {
                            ...
                            const current = localTask.subtasks ?? currentTask.subtasks ?? [];
                            const next = [...current, {
                              id: generateId(),
                            ...
                            updateLocalField('subtasks', next);
                            setSubtaskTitle('');
                            setSelectedSubtaskAssigneeId(null);
                          }
                        }}
                  ...
                  <div className="space-y-1.5">
                    {(localTask.subtasks ?? currentTask.subtasks ?? []).map((subtask) => {
                      ...
                      return (
                        <div key={subtask.id} className="group flex items-center gap-2 rounded-xl border border-surface-100 bg-surface-50 p-2.5 transition-all hover:border-surface-200 dark:border-surface-700 dark:bg-surface-800 dark:hover:border-surface-600">
                          <input
                            ...
                            onChange={() => {
                              const current = localTask.subtasks ?? currentTask.subtasks ?? [];
                              const next = current.map(s => s.id === subtask.id ? { ...s, isCompleted: !s.isCompleted } : s);
                              updateLocalField('subtasks', next);
                            }}
                                  ...
                                  <div className="max-h-40 overflow-y-auto">
                                    {(project ? users.filter(u => project.members.includes(u.id) && ['team_leader', 'team_member'].includes(u.role)) : users.filter(u => ['team_leader', 'team_member'].includes(u.role))).map(u => (
                                      <button
                                        ...
                                        onClick={() => {
                                          const current = localTask.subtasks ?? currentTask.subtasks ?? [];
                                          const next = current.map(s => s.id === subtask.id ? { ...s, assigneeId: u.id } : s);
                                          updateLocalField('subtasks', next);
                                          setEditingSubtaskId(null);
                                        }}
                                    ...
                                    <button
                                      ...
                                      onClick={() => {
                                        const current = localTask.subtasks ?? currentTask.subtasks ?? [];
                                        const next = current.map(s => s.id === subtask.id ? { ...s, assigneeId: null } : s);
                                        updateLocalField('subtasks', next);
                                        setEditingSubtaskId(null);
                                      }}
                          ...
                          <button
                            ...
                            onClick={() => {
                              const current = localTask.subtasks ?? currentTask.subtasks ?? [];
                              const next = current.filter(s => s.id !== subtask.id);
                              updateLocalField('subtasks', next);
                            }}
                    ...
                    {((localTask.subtasks ?? currentTask.subtasks ?? []).length === 0) && <p className="text-sm italic text-surface-400">No subtasks yet</p>}
                  </div>
                </div>
                <div>
                  <div className="mb-2 flex items-center justify-between">
          ...
          <div className="w-full lg:w-64 border-t lg:border-t-0 lg:border-l border-surface-100 dark:border-surface-800 p-3 sm:p-4 flex flex-col gap-3 flex-shrink-0 bg-surface-50/50 dark:bg-surface-950/30 overflow-y-auto">
              ...
              <div className="relative">
                <select
                  ...
                  onChange={(e) => {
                    ...
                    if ((nextStatus === 'done' || nextStatus === 'in_review') && pendingSubtasks) {
                      setPendingSubtasksModalOpen(true);
                      return;
                    }
                    updateLocalField('status', nextStatus);
                  }}
                  className={cn(
                ...
                >
                  {Object.entries(STATUS_CONFIG).map(([key, val]) => (
                    <option key={key} value={key} disabled={(key === 'done' || key === 'in_review') && pendingSubtasks}>
                      {val.label}
                    </option>
                  ))}
                </select>
                              ...
                              <div className="flex gap-1 pt-1">
                                <button
                                  onClick={async () => {
                                    ...
                                    try {
                                      const response = await labelsService.create({ name: newLabelName.trim(), color: newLabelColor });
                                      const newL = response.data.data ?? response.data;
                                      await bootstrap();
                                      const currentIds = (localTask.labels ?? currentTask.labels ?? []).map(l => typeof l === 'object' ? l.id : l);
                                      updateLocalField('labels', [...currentIds, newL.id]);
                                      setNewLabelName('');
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5 items-center">
                {(localTask.tags ?? currentTask.tags ?? []).map(tag => (
                  <span key={tag} className="px-2 py-0.5 rounded-md bg-surface-100 dark:bg-surface-800 border border-surface-200 dark:border-surface-700 text-[10px] font-medium text-surface-600 dark:text-surface-400 group relative">
                    ...
                    {canAddSubtask && (
                      <button
                        onClick={() => {
                          const current = localTask.tags ?? currentTask.tags ?? [];
                          const next = current.filter(t => t !== tag);
                          updateLocalField('tags', next);
                        }}
                        className="absolute -right-1 -top-1 w-3.5 h-3.5 rounded-full bg-rose-500 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity shadow-sm"
                      >
                        <X size={8} strokeWidth={3} />
        ...
        <Modal open={pendingSubtasksModalOpen} onClose={() => setPendingSubtasksModalOpen(false)} size="md">
          <div className="p-8">
            <div className="flex flex-col items-center text-center mb-8">
              <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-amber-50 dark:bg-amber-950/30 text-amber-500 mb-4 shadow-sm border border-amber-100 dark:border-amber-900/20">
                <AlertTriangle size={32} strokeWidth={1.5} />
              </div>
              <h3 className="text-xl font-extrabold text-surface-900 dark:text-white mb-2 tracking-tight">
                Pending Subtasks
              </h3>
              <p className="text-sm text-surface-500 max-w-[280px] leading-relaxed">
                You cannot complete this task until all associated subtasks are finished.
              </p>
            </div>

            <div className="mb-8 overflow-hidden rounded-2xl border border-surface-100 bg-surface-50/30 dark:border-surface-800 dark:bg-surface-950/40">
              <div className="px-4 py-2.5 bg-surface-100/50 dark:bg-surface-800/50 border-b border-surface-100 dark:border-surface-800">
                <span className="text-[10px] font-bold uppercase tracking-widest text-surface-400">Items to complete</span>
              </div>
              <div className="p-2 max-h-[240px] overflow-y-auto custom-scrollbar">
                {(localTask.subtasks ?? currentTask.subtasks ?? []).filter(s => !s.isCompleted).map(subtask => (
                  <div key={subtask.id} className="flex items-center gap-3 p-3 rounded-xl hover:bg-white dark:hover:bg-surface-900 transition-colors group">
                    <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-lg border border-amber-200 bg-amber-50 text-amber-600 dark:border-amber-900/30 dark:bg-amber-950/40">
                      <ListTodo size={14} />
                    </div>
            ...
            <div className="flex flex-col gap-3">
              <button
                onClick={() => setPendingSubtasksModalOpen(false)}
                className="btn-primary w-full h-12 rounded-xl text-sm font-bold shadow-xl shadow-brand-500/20 active:scale-[0.98] transition-all"
              >
                Continue Working
              </button>
            </div>
