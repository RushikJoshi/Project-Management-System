...
export const Dropdown: React.FC<DropdownProps> = ({
  value, onChange, items, placeholder = 'Select...', label, className, triggerClassName, disabled, icon, hideLabel, hideChevron
}) => {
        ...
        >
          <span className={cn("truncate", !selected && "text-surface-400")}>
            <span className="flex items-center gap-2">
              {icon || selected?.icon}
              {!hideLabel && (selected ? selected.label : placeholder)}
            </span>
          </span>
          {!hideChevron && <ChevronDown size={14} className="text-surface-400 flex-shrink-0" />}
        </RadixDropdown.Trigger>
...
export const DatePicker: React.FC<DatePickerProps> = ({
  value, onChange, label, placeholder = 'Select date...', minDate, className, disabled
}) => {
      ...
      <RadixPopover.Root>
        <RadixPopover.Trigger asChild>
          <button
            disabled={disabled}
            className={cn(
              "flex items-center justify-between gap-2 px-3 py-2 rounded-xl border bg-white dark:bg-surface-900 border-surface-100 dark:border-surface-800 hover:border-brand-300 transition-all outline-none text-xs font-semibold",
              "disabled:opacity-50 disabled:cursor-not-allowed"
            )}
          >
...
export const TimePicker: React.FC<{
  ...
  const onOpenChange = (open: boolean) => {
    if (open) {
      setTimeout(() => {
        hourRefs.current[hour]?.scrollIntoView({ block: 'nearest', behavior: 'auto' });
        minuteRefs.current[minute]?.scrollIntoView({ block: 'nearest', behavior: 'auto' });
      }, 50);
    }
  };
    ...
    <div className={cn('flex flex-col gap-1', className)}>
      ...
      <RadixPopover.Root onOpenChange={onOpenChange}>
        <RadixPopover.Trigger asChild disabled={disabled}>
          <button className="flex items-center justify-between gap-2 px-3 py-2 rounded-xl border bg-white dark:bg-surface-900 border-surface-100 dark:border-surface-800 hover:border-brand-300 transition-all outline-none text-xs font-bold min-w-[95px]">
            <span>{value}</span>
        ...
        <RadixPopover.Portal>
          <RadixPopover.Content 
            align="start" 
            sideOffset={4} 
            className="z-[9999] bg-white dark:bg-surface-900 rounded-2xl border border-surface-100 dark:border-surface-800 shadow-2xl p-0 flex h-[240px] overflow-hidden animate-in fade-in zoom-in-95 duration-150"
          >
            <div className="w-[70px] h-full overflow-y-auto py-1 border-r border-surface-50 dark:border-surface-800 scrollbar-auto">
              {hours.map(h => (
                <button
                  key={h}
            ...
            <div className="w-[70px] h-full overflow-y-auto py-1 scrollbar-auto">
              {minutes.map(m => (
                <button
                  key={m}
