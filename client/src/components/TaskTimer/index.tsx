...
import { tasksService, timeTrackingService } from '../../services/api';
...
export const TaskTimer: React.FC<TaskTimerProps> = ({ task, isReadOnly }) => {
  ...
  // Find the active log if any - now pulling from the task's time logs which are synced
  ...
  const toggleTimer = async () => {
    ...
    try {
      if (activeLog) {
        // Stop Timer
        const response = await timeTrackingService.stop(task.id);
        if (task.status === 'in_progress') {
          await tasksService.move(task.id, 'todo');
        }
        emitSuccessToast('Timer stopped and logged.');
      } else {
        // Start Timer
        await timeTrackingService.start(task.id);
        if (task.status !== 'in_progress') {
          await tasksService.move(task.id, 'in_progress');
        }
        emitSuccessToast('Timer started.');
      }
      
      // Full refresh to get latest task state with logs
      await bootstrap();
    } catch (err: any) {
