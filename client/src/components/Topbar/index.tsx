...
export const Topbar: React.FC = () => {
  ...
  const { darkMode, toggleDarkMode, sidebarCollapsed, unreadNotificationsCount, projects, tasks, quickTasks, users, toggleMobileSidebar } = useAppStore();
  ...
  const breadcrumbs = segments.map((seg, i) => {
    ...
    // 1. Check if it's a project ID
    const project = projects.find(p => p.id === seg);
    if (project) return { label: project.name, path };

    // 2. Check if it's a task ID (Standard Project Tasks)
    const task = tasks.find(t => t.id === seg);
    if (task) return { label: task.title, path };

    // 3. Check if it's a quick task ID
    const quickTask = quickTasks.find(qt => qt.id === seg);
    if (quickTask) return { label: 'Quick Task', path };

    // 4. Fallback to Breadcrumb Map or raw segment
            ...
            {profileOpen && (
              ...
              >
                ...
                <div className="p-2">
                  ...
                  <WorkLogoutButton className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-sm text-rose-600 transition-colors hover:bg-rose-50 dark:hover:bg-rose-950/20">
                    <LogOut size={15} />
                    Logout
                  </WorkLogoutButton>
                </div>
