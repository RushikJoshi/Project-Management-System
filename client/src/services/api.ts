...
api.interceptors.request.use(
  (config: InternalAxiosRequestConfig) => {
    ...
    const workspaceId = auth?.state?.user?.workspaceId;
    if (workspaceId) {
...
api.interceptors.response.use(
  ...
  async (error: AxiosError) => {
    ...
    if (status === 401 && originalRequest && !originalRequest.__skipAuthRefresh && !originalRequest._retry && !isLoginPage) {
...
export const tasksService = {
  ...
  update: (id: string, data: unknown, config?: any) => api.put(`/tasks/${id}`, data, config),
  delete: (id: string) => api.delete(`/tasks/${id}`),
  getOverdue: () => api.get('/tasks/overdue'),
  getStats: () => api.get('/tasks/stats'),
  getUnifiedList: (params: {
    type?: 'project' | 'quick';
    filter?: 'all' | 'active' | 'overdue' | 'completed';
    page?: number;
    limit?: number;
    searchTerm?: string;
    department?: string;
    personId?: string;
  }) => api.get('/tasks/unified-list', { params }),
...
export const workSessionsService = {
  getMySummary: () => api.get('/work-sessions/me'),
  logActivity: (data: { sessionId: string; activityType: string; taskId?: string; description?: string }) =>
    api.post('/work-sessions/activity', data),
  checkPendingTasks: () => api.get('/work-sessions/pending-tasks'),
  logout: (data: { sessionId: string; option: 'direct' | 'submit_reason' | 'request_extension'; data?: { tasks?: Array<{ taskId: string; reason: string; blockerType: string; expectedCompletion: string }> } }) =>
    api.post('/work-sessions/logout', data, { suppressErrorToast: true }),
  getPendingLogoutReports: (params?: { date?: string; department?: string }) =>
    api.get('/work-sessions/manager/pending-logout', { params }),
  getTeamProductivity: (params?: { date?: string; department?: string }) =>
    api.get('/work-sessions/manager/productivity', { params }),
  reviewExtension: (id: string, body: { action: 'approve' | 'reject'; comment?: string }) =>
    api.patch(`/work-sessions/manager/extensions/${id}`, body),
};
...
export const clientsService = {
  ...
  create: (data: unknown) => api.post('/clients', data, { idempotencyKey: createIdempotencyKey('client-create') }),
  update: (id: string, data: unknown) => api.put(`/clients/${id}`, data),
  invite: (clientId: string, data: { email: string; role: string; projectIds?: string[] }) =>
    api.post(`/clients/${clientId}/invite`, data),
  assignProjects: (clientId: string, projectIds: string[]) =>
    api.post(`/clients/${clientId}/projects`, { projectIds }),
};
...
export const ticketsService = {
  ...
  updateStatus: (id: string, status: string, note?: string, taskId?: string) =>
    api.patch(`/tickets/${id}/status`, { status, note, taskId }),
  addComment: (id: string, data: { content: string; isInternal?: boolean; attachments?: any[] }) =>
...
export const teamsService = {
  ...
  getWorkload: (id: string) => api.get(`/teams/${id}/workload`),
...
export const timeTrackingService = {
  ...
  getTaskLogs: (taskId: string) => api.get(`/time-tracking/task/${taskId}`),
  start: (taskId: string) => api.post('/time-tracking/start', { taskId }),
  stop: (taskId: string) => api.post('/time-tracking/stop', { taskId }),
  addManual: (data: { taskId: string; durationMinutes: number; notes?: string }) => api.post('/time-tracking/manual', data),
};

export const performanceService = {
  getDiscovery: () => api.get('/performance/discovery'),
  getMe: (days = 30) => api.get('/performance/me', { params: { days } }),

  getWorkspace: (days = 30) => api.get('/performance/workspace', { params: { days } }),
  getUser: (userId: string, days = 30) => api.get(`/performance/user/${userId}`, { params: { days } }),
  getTeam: (teamId: string, days = 30) => api.get(`/performance/team/${teamId}`, { params: { days } }),
};
...
export const authService = {
  ...
  logout: (refreshToken?: string | null) => api.post('/auth/logout', refreshToken ? { refreshToken } : {}, { __skipAuthRefresh: true, suppressErrorToast: true }),
  refresh: (refreshToken: string) => api.post('/auth/refresh', { refreshToken }, { __skipAuthRefresh: true }),
  verifyClientInvite: (tenantId: string, token: string) => api.get(`/auth/client-invite/${tenantId}/${token}`),
  acceptClientInvite: (tenantId: string, token: string, data: any) => api.post(`/auth/client-invite/${tenantId}/${token}`, data),
};
...
export const rbacService = {
  getRegistry: () => api.get('/rbac/registry'),
  createCustomPermission: (data: unknown) => api.post('/rbac/registry/custom', data, { idempotencyKey: createIdempotencyKey('rbac-custom-perm-create') }),
  getRoles: () => api.get('/rbac/roles'),
  createRole: (data: unknown) => api.post('/rbac/roles', data, { idempotencyKey: createIdempotencyKey('rbac-role-create') }),
  updateRole: (id: string, data: unknown) => api.put(`/rbac/roles/${id}`, data),
  deleteRole: (id: string) => api.delete(`/rbac/roles/${id}`),
  getUserPermissions: (userId: string) => api.get(`/rbac/users/${userId}`),
  updateUserPermissions: (userId: string, data: unknown) => api.put(`/rbac/users/${userId}`, data),
};

export const clientDashboardService = {
  getStats: () => api.get('/client-dashboard/stats'),
  getProjects: () => api.get('/client-dashboard/projects'),
  getActivity: () => api.get('/client-dashboard/activity'),
};

export const clientTeamsService = {
  getSummary: () => api.get('/client-teams/summary'),
