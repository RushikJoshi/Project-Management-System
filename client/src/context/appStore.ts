...
interface AppStore {
  ...
  taskStats: {
    projectTasks: { active: number; overdue: number; completed: number; total: number };
    quickTasks: { active: number; overdue: number; completed: number; total: number };
  } | null;
  teamStats: {
    totalTeams: number;
    totalLeads: number;
...
export const useAppStore = create<AppStore>((set, get) => ({
  ...
  bootstrap: async () => {
    ...
    const [usersRes, workspacesRes, projectsRes, tasksRes, teamsRes, quickRes, notifRes, personalRes, labelsRes, statsRes, teamStatsRes] = await Promise.all([
      usersService.getAll(),
    ...
    set({
      ...
      taskStats: statsRes.data.data || null,
      teamStats: teamStatsRes.data.data || null,
    });
  },
  ...
  fetchStats: async () => {
    try {
      ...
      set({ taskStats: res.data.data || null });
    } catch (err) {
  ...
  fetchTeamStats: async () => {
    try {
      const res = await teamsService.getStats();
      set({ teamStats: res.data.data || null });
    } catch (err) {
  ...
  addProject: (project) => {
    set(s => ({ projects: [...s.projects, { ...project, id: project.id || (project as any)._id }] }));
    get().fetchTeamStats();
  },
