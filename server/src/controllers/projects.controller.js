...
export async function get(req, res, next) {
  try {
    const { companyId, workspaceId, sub: userId, role, userType, clientId } = req.auth;
    const project = await ProjectService.getProject({ companyId, workspaceId, projectId: req.params.id, userId, role, userType, clientId });
...
export async function update(req, res, next) {
  try {
    const { companyId, workspaceId, sub: userId, role, userType, clientId } = req.auth;
    const project = await ProjectService.updateProject({ companyId, workspaceId, userId, role, projectId: req.params.id, updates: req.body, userType, clientId });
    if (!project) return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: 'Project not found' } });
    return res.status(200).json({ success: true, data: project });
  } catch (e) {
    return next(e);
...
export async function remove(req, res, next) {
  try {
    const { companyId, workspaceId, sub: userId, role, userType, clientId } = req.auth;
    const project = await ProjectService.deleteProject({ companyId, workspaceId, userId, role, projectId: req.params.id, userType, clientId });
    if (!project) return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: 'Project not found' } });
