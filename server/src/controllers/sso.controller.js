...
export async function me(req, res) {
  try {
    ...
    try {
      ...
      return res.status(200).json({ user: null, message: 'Invalid or expired session' });
    }
    ...
    if (freshUser === null && freshUser !== undefined) {
      // Explicit null means user was found but condition failed (e.g. inactive)
      return res.status(200).json({ user: null, message: 'User not found or inactive' });
    }
