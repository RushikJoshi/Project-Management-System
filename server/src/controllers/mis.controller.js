...
async function buildMisVisibilityQuery({ companyId, workspaceId, userId, role }) {
  if (role === 'super_admin' || role === 'admin') return {};
  
  const { getAccessibleProjectIds, getTeamMemberIdsForLeader } = await import('../services/task.service.js');
  
  if (role === 'manager') {
    const projectIds = await getAccessibleProjectIds({ tenantId: companyId, workspaceId, userId, role });
    ...
    const memberIds = await getTeamMemberIdsForLeader({ tenantId: companyId, workspaceId, userId });
    return {
      $or: [
        { employeeId: userId },
...
export async function getPendingMIS(req, res, next) {
  try {
    ...
    const visibilityQuery = await buildMisVisibilityQuery({ companyId, workspaceId, userId, role });
...
export async function getReportWeekly(req, res, next) {
  try {
    ...
    const visibilityQuery = await buildMisVisibilityQuery({ companyId, workspaceId, userId, role });
    const query = { tenantId: companyId, ...visibilityQuery };
...
export async function getReportEmployee(req, res, next) {
  try {
    ...
    const pipelines = [
      { $match: { tenantId: new mongoose.Types.ObjectId(companyId), ...visibilityQuery } },
      { $unwind: "$keyTasks" },
      {
        $group: {
          _id: "$employeeId",
    ...
    // Also filter the list of users returned to only those within the visibility bounds,
    // or just rely on the aggregation result to only populate what they have access to.
    const validEmployeeIds = stats.map(s => String(s._id));
...
export async function getReportProject(req, res, next) {
  try {
    ...
    const pipelines = [
      { $match: { tenantId: new mongoose.Types.ObjectId(companyId), projectId: { $exists: true, $ne: null }, ...visibilityQuery } },
      { $unwind: "$keyTasks" },
      {
        $group: {
          _id: {
