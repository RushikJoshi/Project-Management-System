...
import { requireAuth, requireInternalUser } from '../../../middleware/auth.middleware.js';
...
router.get('/stats', TaskStatsController.getStats);
router.get('/unified-list', TaskStatsController.getUnifiedList);
...
const taskTypeEnum = z.string();
const timelineTypeEnum = z.enum(['task', 'milestone']);
const objectId = z.string(); // Relaxed to allow client-side generated IDs
...
const taskCreateSchema = z.object({
  ...
  createdFromRequest: z.boolean().optional(),
});
...
router.get('/requests', requireInternalUser, TasksController.listTaskRequests);
router.post('/requests', requireInternalUser, mutationRateLimiter, enforceIdempotency(), validateBody(taskCreateSchema), TasksController.createTaskRequest);
router.post('/requests/:id/review', requireInternalUser, validateBody(taskRequestReviewSchema), TasksController.reviewTaskRequest);
...
router.post('/:id/comments', authorize('client.request.comment'), validateBody(addCommentSchema), TasksController.addComment);
