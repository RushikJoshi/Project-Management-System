...
import { ensureAutoTeamForProject } from './team.service.js';
...
function buildClientProjectAccessFilter({ clientId }) {
  return {
    clientId,
...
export async function listProjects({ companyId, workspaceId, userId, role, status, department, q, page = 1, limit = 50, userType, clientId }) {
  const tenantId = companyId;
  const { Project } = await getTenantModels(companyId);
  
  let filter;
  if (userType === 'client' || isClientRole(role)) {
    filter = { tenantId, workspaceId, ...buildClientProjectAccessFilter({ clientId }) };
  } else {
    ...
    filter = { tenantId, workspaceId, ...(hasGlobalVisibility ? {} : buildOwnedProjectAccessFilter({ userId })) };
  }
...
export async function getProject({ companyId, workspaceId, projectId, userId, role, userType, clientId }) {
  const tenantId = companyId;
  const { Project } = await getTenantModels(companyId);
  
  let accessFilter;
  if (userType === 'client' || isClientRole(role)) {
    accessFilter = buildClientProjectAccessFilter({ clientId });
  } else {
    const hasGlobalVisibility = isProjectAdminRole(role) || await canSeeOtherProjects({ companyId, workspaceId, role }) || await canEditOtherProjects({ companyId, workspaceId, role });
    accessFilter = hasGlobalVisibility ? {} : buildOwnedProjectAccessFilter({ userId });
  }
...
export async function createProject({ companyId, workspaceId, userId, role, data }) {
  ...
  const leadId = data.leadId && mongoose.Types.ObjectId.isValid(data.leadId) ? data.leadId : userId;
  ...
  try {
    await ensureAutoTeamForProject({
      companyId,
      workspaceId,
      userId,
      projectId: project._id,
    ...
    logger.error('project_auto_team_failed', { projectId: String(project._id), error: err.message });
  }

  return project;
}

export async function updateProject({ companyId, workspaceId, userId, role, projectId, updates, userType, clientId }) {
  const tenantId = companyId;
  const { Project, ActivityLog } = await getTenantModels(companyId);
  
  let accessFilter;
  if (userType === 'client' || isClientRole(role)) {
    // Clients can usually only view, but maybe CLIENT_ADMIN can update some fields?
    // For now, only allow update if they have access.
    accessFilter = buildClientProjectAccessFilter({ clientId });
  } else {
    const hasGlobalEdit = isProjectAdminRole(role) || await canEditOtherProjects({ companyId, workspaceId, role });
    accessFilter = hasGlobalEdit ? {} : buildOwnedProjectAccessFilter({ userId });
  }
  ...
  const project = await Project.findOneAndUpdate(
    { _id: projectId, tenantId, workspaceId, ...accessFilter },
    {
      $set: {
        ...normalizedUpdates,
  ...
  try {
    ...
    logger.error('project_auto_team_update_failed', { projectId: String(project._id), error: err.message });
  }

  return project;
}

export async function deleteProject({ companyId, workspaceId, userId, role, projectId, userType, clientId }) {
  const tenantId = companyId;
  const { Project, ActivityLog } = await getTenantModels(companyId);
  ...
  if (userType === 'client' || isClientRole(role)) {
    ...
    const err = new Error('Clients are not allowed to delete projects');
    err.statusCode = 403;
    throw err;
  } else {
    const hasGlobalEdit = isProjectAdminRole(role) || await canEditOtherProjects({ companyId, workspaceId, role });
