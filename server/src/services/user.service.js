...
export async function createUser({ companyId, workspaceId, actorRole, input }) {
  ...
  const { User, Membership, Workspace, Role } = await getTenantModels(companyId);
  ...
  // Handle role transition: if roleId is provided but role is not
  let targetRole = input.role;
  let targetRoleIds = input.roleIds || [];
  if (input.roleId && !targetRoleIds.includes(input.roleId)) {
    targetRoleIds.push(input.roleId);
  }

  if (targetRoleIds.length > 0 && !targetRole) {
    const roleDocs = await Role.find({ _id: { $in: targetRoleIds }, tenantId: companyId }).lean();
    if (roleDocs.length > 0) {
      // Find the first role that matches a system role slug, or default to the first one's slug if it fits the enum
      const systemRoles = ['super_admin', 'admin', 'manager', 'team_leader', 'team_member'];
      const systemRoleMatch = roleDocs.find(r => systemRoles.includes(r.slug));
      if (systemRoleMatch) {
        targetRole = systemRoleMatch.slug;
      } else {
        // If no system role match, use 'team_member' as legacy fallback
        targetRole = 'team_member';
      }
    }
  }

  if (!targetRole) targetRole = 'team_member';
  ...
  await Membership.updateOne(
    ...
    { $setOnInsert: { role: targetRole, roleIds: targetRoleIds, status: 'active' } },
    { upsert: true }
  );
...
export async function importUsersBulk({ companyId, workspaceId, actorRole, rows }) {
  ...
  for (let index = 0; index < normalizedRows.length; index += 1) {
    ...
    const input = {
      ...
      roleIds: Array.isArray(row.roleIds) ? row.roleIds : undefined,
...
export async function updateUser({ companyId, workspaceId, actorRole, userId, targetUserId, updates }) {
  ...
  let targetRole = updates.role;
  let targetRoleIds = updates.roleIds || (updates.roleId ? [updates.roleId] : undefined);

  if (targetRoleIds && targetRoleIds.length > 0 && !targetRole) {
    ...
    if (roleDocs.length > 0) {
      ...
      if (systemRoleMatch) {
        ...
        targetRole = 'team_member'; // default fallback
      }
    }
  }

  if (String(user._id) === String(userId) && targetRole && targetRole !== user.role) {
  ...
  if (targetRoleIds) user.roleIds = targetRoleIds;
  ...
  await Membership.updateMany(
    { tenantId, userId: user._id, ...(workspaceId ? { workspaceId } : {}) },
    {
      $set: {
        ...
        ...(targetRoleIds ? { roleIds: targetRoleIds } : {}),
