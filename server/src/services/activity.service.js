...
export async function listActivity({ companyId, workspaceId, userId, role, limit = 50, q, type, entityType, days }) {
  const tenantId = companyId;
  const { ActivityLog, User, Task, QuickTask } = await getTenantModels(companyId);

  const filter = { tenantId, workspaceId };

  if (role !== 'super_admin' && role !== 'admin') {
    const { buildVisibilityQuery, getAccessibleProjectIds } = await import('./task.service.js');
    const visibilityQuery = await buildVisibilityQuery({ tenantId, workspaceId, userId, role });
    
    const [visibleTasks, visibleQuickTasks, accessibleProjectIds] = await Promise.all([
      Task.find({ tenantId, workspaceId, ...visibilityQuery }).select('_id').lean(),
      QuickTask.find({ tenantId, workspaceId, ...visibilityQuery }).select('_id').lean(),
      getAccessibleProjectIds({ tenantId, workspaceId, userId, role })
    ]);

    const accessibleEntityIds = [
      ...visibleTasks.map(t => String(t._id)),
      ...visibleQuickTasks.map(t => String(t._id))
    ];

    const entityClauses = [
      { entityType: { $in: ['TASK', 'task', 'quick_task', 'QUICK_TASK', 'task_request'] }, entityId: { $in: accessibleEntityIds } },
      // allow users to see activities where they are the actor
      { userId }
    ];

    if (accessibleProjectIds === null) {
      entityClauses.push({ entityType: { $in: ['PROJECT', 'project', 'team', 'TEAM'] } });
    } else if (accessibleProjectIds.length > 0) {
      entityClauses.push({ entityType: { $in: ['PROJECT', 'project'] }, entityId: { $in: accessibleProjectIds } });
      entityClauses.push({ 'metadata.projectId': { $in: accessibleProjectIds } });
    }

    filter.$and = [{ $or: entityClauses }];
  }
