...
import { 
  isTaskOverdue, 
  getOverdueQueryFilter, 
  ACTIVE_STATUSES, 
  TERMINAL_STATUSES,
  getActiveQueryFilter 
} from '../utils/task.utils.js';
import { syncTeamStats } from './team.service.js';

async function syncProjectAndTeamStats({ companyId, workspaceId, projectId }) {
  if (!projectId) return;
  await syncProjectStats(companyId, workspaceId, projectId);
  
  const { Project } = await getTenantModels(companyId);
  const p = await Project.findById(projectId).select('teamId').lean();
  if (p?.teamId) {
    await syncTeamStats({ companyId, workspaceId, teamId: p.teamId });
  }
}

export async function getTaskStatisticsUnified({ companyId, workspaceId, userId, role, userType, clientId }) {
  ...
  const overdueFilter = getOverdueQueryFilter();
  const visibilityQuery = await buildVisibilityQuery({ tenantId, workspaceId, userId, role, userType, clientId });

  // Project Task Aggregation
  const projectBaseFilter = { 
    tenantId: tid, 
    workspaceId: wid, 
    isDeleted: { $ne: true },
    ...visibilityQuery 
  ...
  const [pStats] = await Task.aggregate([
    { $match: projectBaseFilter },
    {
      $facet: {
        active: [{ $match: { status: { $in: ACTIVE_STATUSES } } }, { $count: "count" }],
        overdue: [{ $match: overdueFilter }, { $count: "count" }],
        completed: [{ $match: { status: { $in: ['done', 'completed'] } } }, { $count: "count" }],
        total: [{ $count: "count" }]
      }
    }
  ]);

  // Quick Task Aggregation
  const quickBaseFilter = { 
    tenantId: tid, 
    workspaceId: wid, 
    isDeleted: { $ne: true },
    ...visibilityQuery 
  ...
  const [qStats] = await QuickTask.aggregate([
    { $match: quickBaseFilter },
    {
      $facet: {
        active: [{ $match: { status: { $in: ACTIVE_STATUSES } } }, { $count: "count" }],
        overdue: [{ $match: overdueFilter }, { $count: "count" }],
  ...
  return {
    projectTasks: {
      ...
      overdue: pStats.overdue[0]?.count || 0,
      completed: pStats.completed[0]?.count || 0,
      total: pStats.total[0]?.count || 0
    },
    quickTasks: {
      active: qStats.active[0]?.count || 0,
      overdue: qStats.overdue[0]?.count || 0,
      completed: qStats.completed[0]?.count || 0,
      total: qStats.total[0]?.count || 0
    }
  };
}

export async function listTasksUnified({ 
  companyId, 
  workspaceId, 
  userId, 
  role, 
  ...
  const tenantId = companyId;
  const { Task, QuickTask, User } = await getTenantModels(tenantId);
  ...
  // 1. Build Base Visibility Filter
  let query = { tenantId: tid, workspaceId: wid, isDeleted: { $ne: true } };

  const visibilityQuery = await buildVisibilityQuery({ tenantId, workspaceId, userId, role, userType, clientId });
  Object.assign(query, visibilityQuery);

  // 2. Apply Status/Category Filters
  if (filter === 'active') {
    query.status = { $in: ACTIVE_STATUSES };
  } else if (filter === 'overdue') {
    Object.assign(query, getOverdueQueryFilter());
  } else if (filter === 'completed') {
    query.status = { $in: ['done', 'completed'] };
  }

  // 3. Apply Advanced Filters (Search, Person, Department)
  if (searchTerm) {
    query.title = { $regex: searchTerm, $options: 'i' };
  }

  if (personId && personId !== 'all') {
    const targetUid = new mongoose.Types.ObjectId(personId);
    query.$and = query.$and || [];
    query.$and.push({
      $or: [
        { assigneeIds: targetUid },
        { reporterId: targetUid }
      ]
    });
  }
  ...
  if (department && department !== 'all') {
    ...
    const deptUsers = await User.find({ department }).select('_id').lean();
    const deptUserIds = deptUsers.map(u => u._id);
    query.$and = query.$and || [];
    query.$and.push({
      $or: [
        { assigneeIds: { $in: deptUserIds } },
        { reporterId: { $in: deptUserIds } }
      ]
    });
  }
  ...
  const [items, total] = await Promise.all([
    Model.find(query)
      ...
      .populate('reporterId', 'name avatar color')
      .skip(skip)
      .limit(limit)
      .lean(),
  ...
  // 5. Get counts for the current context (Project vs Quick)
  const stats = await getTaskStatisticsUnified({ companyId, workspaceId, userId, role, userType, clientId });
  ...
  return {
    ...
    page,
    limit,
    counts: type === 'project' ? stats.projectTasks : stats.quickTasks
  };
}
...
function isAdminRole(role) {
  return ['super_admin', 'admin', 'manager', 'owner', 'workspace_admin'].includes(role);
}
...
function canApproveTaskRequest({ role, userId, project }) {
  ...
  if (project && (String(project.ownerId) === uid || String(project.leadId) === uid)) return true;
  return false;
}

function isClientRole(role) {
  return typeof role === 'string' && role.startsWith('CLIENT_');
}

async function getTeamMemberIdsForLeader({ tenantId, workspaceId, userId }) {
  const { Team } = await getTenantModels(tenantId);
  const uid = new mongoose.Types.ObjectId(userId);
  ...
  teams.forEach(team => {
    (team.members || []).forEach(m => memberIds.add(String(m)));
  });
  return Array.from(memberIds).map(id => new mongoose.Types.ObjectId(id));
}

export async function buildVisibilityQuery({ tenantId, workspaceId, userId, role, userType, clientId }) {
  if (role === 'super_admin' || role === 'admin') return {};

  const uid = new mongoose.Types.ObjectId(userId);
  
  if (userType === 'client' || isClientRole(role)) {
  ...
  // Base visibility for employees: only tasks explicitly assigned to them
  if (role === 'team_member' || role === 'employee') {
    return { assigneeIds: uid };
  }

  // Base visibility for other non-admins: own tasks
  const baseConditions = [
    { assigneeIds: uid }
  ];

  if (role === 'manager') {
    // MANAGER: own tasks + tasks under projects they manage
    const { Project } = await getTenantModels(tenantId);
    ...
    if (managedProjects.length > 0) {
      ...
      baseConditions.push({ projectId: { $in: projectIds } });
    }
  } else if (role === 'team_leader' || role === 'teamleader') {
    // TEAM LEADER: own tasks + team members' tasks
    const memberIds = await getTeamMemberIdsForLeader({ tenantId, workspaceId, userId });
    if (memberIds.length > 0) {
      baseConditions.push({ assigneeIds: { $in: memberIds } });
    }
  } else {
    // FALLBACK for any other role: only own tasks
    return { assigneeIds: uid };
  }
...
export async function getAccessibleProjectIds({ tenantId, workspaceId, userId, role, userType, clientId }) {
  ...
  if (userType === 'client' || isClientRole(role)) {
    ...
    return clientProjects.map(p => p._id);
  }
  ...
  if (isAdminRole(role) || canSeeOthers || canEditOthers) return null;
  ...
  for (const p of projects) {
    ...
    if (p.teamId) {
      const team = await Team.findOne({ _id: p.teamId, tenantId }).lean();
      if (
        team && (
...
function taskModifyRoles(role, task, userId) {
  if (isAdminRole(role)) return true;
  if (['manager', 'team_leader', 'teamleader'].includes(role)) return true;
  const uid = strId(userId);
  
  // STRICT RULE for employees: only assignee can modify
  if (role === 'team_member' || role === 'employee') {
    return (task.assigneeIds || []).some((a) => strId(a) === uid);
  }
...
function normalizeObjectIdInput(value) {
  const raw = strId(value).trim();
  if (!raw || raw === 'undefined' || raw === 'null') return '';
  return raw;
}

function assertValidObjectId(value, fieldName, message) {
  const normalized = normalizeObjectIdInput(value);
  if (!normalized) {
    const err = new Error(message || `${fieldName} is required`);
    err.statusCode = 400;
    err.code = `${fieldName.toUpperCase()}_REQUIRED`;
    throw err;
  }
  if (!mongoose.Types.ObjectId.isValid(normalized)) {
    const err = new Error(`Invalid ${fieldName} format`);
    err.statusCode = 400;
    err.code = `INVALID_${fieldName.toUpperCase()}_FORMAT`;
    throw err;
  }
  return normalized;
}
...
export async function assertProjectAccess({ tenantId, workspaceId, userId, role, projectId, userType, clientId }) {
  ...
  if (!project) {
    const err = new Error(`Project not found in assertProjectAccess (ID: ${projectId}, Tenant: ${tenantId}, Workspace: ${workspaceId})`);
    err.statusCode = 404;
    err.code = 'PROJECT_NOT_FOUND';
    throw err;
  }

  const allowed = await getAccessibleProjectIds({ tenantId: tenantId, workspaceId, userId, role, userType, clientId });
...
export async function buildTaskVisibilityFilter({
  ...
  const visibilityQuery = await buildVisibilityQuery({ tenantId, workspaceId, userId, role, userType, clientId });
  Object.assign(base, visibilityQuery);
  ...
  if (status) {
    if (status === 'overdue') {
      Object.assign(base, getOverdueQueryFilter());
    } else if (status === 'active') {
      Object.assign(base, getActiveQueryFilter());
    } else {
      base.status = status;
    }
  }
...
export async function createTask({ companyId, workspaceId, userId, role, data, userType, clientId }) {
  const tenantId = companyId;
  const { Task, Project, ActivityLog, Notification, Ticket } = await getTenantModels(companyId);

  const projectId = assertValidObjectId(data?.projectId, 'projectId', 'Task missing projectId');
  ...
  const requestId = data.requestId ? assertValidObjectId(data.requestId, 'requestId') : null;
  if (requestId) {
    const sourceRequest = await Ticket.findById(requestId).lean();
    if (!sourceRequest) {
      const err = new Error('Linked request not found');
      err.statusCode = 404;
      err.code = 'REQUEST_NOT_FOUND';
      throw err;
    }
    if (!sourceRequest.projectId) {
      const err = new Error('Request has no projectId');
      err.statusCode = 400;
      err.code = 'REQUEST_PROJECT_MISSING';
      throw err;
    }
    if (strId(sourceRequest.projectId) !== strId(projectId)) {
      const err = new Error('Request projectId does not match task projectId');
      err.statusCode = 400;
      err.code = 'REQUEST_PROJECT_MISMATCH';
      throw err;
    }
  }

  const ok = await assertProjectAccess({ tenantId, workspaceId: project.workspaceId, userId, role, projectId, userType, clientId });
  if (!ok) {
  ...
  if (requestId) {
    const projectUserIds = new Set([
      ...mapIdList(project.members),
      ...mapIdList(project.reportingPersonIds),
      strId(project.ownerId),
      strId(project.leadId),
    ].filter(Boolean));
    const invalidAssignees = normalizedAssigneeIds.filter((assigneeId) => !projectUserIds.has(strId(assigneeId)));
    if (invalidAssignees.length) {
      const err = new Error('Assigned users must belong to the linked project');
      err.statusCode = 400;
      err.code = 'ASSIGNEE_NOT_IN_PROJECT';
      throw err;
    }
  }
  ...
  if (duplicateTask) {
    return (await attachTaskActivity({ companyId, workspaceId: project.workspaceId, tasks: [duplicateTask] }))[0];
  }
  ...
  const task = new Task({
    ...
    order: data.order ?? 0,
    requestId,
    createdFromRequest: Boolean(requestId || data.createdFromRequest),
  ...
  await ActivityLog.create({
    ...
    userId,
    type: 'task_created',
    description: requestId ? `Created task "${task.title}" from request` : `Created task "${task.title}"`,
    entityType: 'task',
    entityId: task._id,
    metadata: { projectId: task.projectId, requestId: requestId || undefined },
  });
  ...
  if (requestId) {
    await ActivityLog.create({
      ...
      description: `Created linked task "${task.title}" from request`,
      entityType: 'ticket',
      entityId: requestId,
      metadata: { projectId: task.projectId, taskId: task._id },
    });
  }
  ...
  await syncProjectAndTeamStats({ companyId, workspaceId: project.workspaceId, projectId: task.projectId });

  return (await attachTaskActivity({ companyId, workspaceId: project.workspaceId, tasks: [task] }))[0];
}

export async function listTaskCreationRequests({ companyId, workspaceId, userId, role, projectId, requestStatus, userType, clientId }) {
  const tenantId = companyId;
  const { TaskCreationRequest, Project } = await getTenantModels(companyId);
  const filter = { tenantId, workspaceId };
  if (projectId) {
    const ok = await assertProjectAccess({ tenantId, workspaceId, userId, role, projectId, userType, clientId });
    if (!ok) {
...
export async function createTaskCreationRequest({ companyId, workspaceId, userId, role, data, userType, clientId }) {
  const tenantId = companyId;
  const projectId = assertValidObjectId(data?.projectId, 'projectId', 'Request missing projectId');
  const ok = await assertProjectAccess({ tenantId, workspaceId, userId, role, projectId, userType, clientId });
  if (!ok) {
    const err = new Error('Forbidden: no access to this project');
    err.statusCode = 403;
...
export async function getTaskById({ companyId, workspaceId, userId, role, taskId, userType, clientId }) {
  const tenantId = companyId;
  const { Task } = await getTenantModels(companyId);
  ...
  const ok = await assertProjectAccess({ tenantId, workspaceId: task.workspaceId, userId, role, projectId: task.projectId, userType, clientId });
  if (!ok && !taskModifyRoles(role, task, userId)) return null;

  // STRICT RULE for employees: only assignee can view!
  if (role === 'team_member' || role === 'employee') {
    const uid = String(userId);
    const isAssignee = (task.assigneeIds || []).some((a) => String(a._id || a) === uid);
...
export async function getAnyTaskById({ companyId, workspaceId, userId, role, taskId, userType, clientId }) {
  if (!mongoose.Types.ObjectId.isValid(taskId)) return null;
  const uid = new mongoose.Types.ObjectId(userId);

  // 1. Try project task
  const projectTask = await getTaskById({ companyId, workspaceId, userId, role, taskId, userType, clientId });
  if (projectTask) {
...
export async function updateTask({ companyId, workspaceId, userId, role, taskId, updates }) {
  ...
  if (updates.status && updates.status !== existing.status) {
    ...
    // Automatically complete subtasks when completing a task
    if ((updates.status === 'done' || updates.status === 'in_review')) {
      const subtasks = updates.subtasks !== undefined ? updates.subtasks : existing.subtasks;
      if (Array.isArray(subtasks) && subtasks.some(s => !s.isCompleted)) {
        subtasks.forEach(s => {
  ...
  if (updates.status && updates.status !== existing.status) {
    if (task && task.requestId) {
      ...
      const ticket = await Ticket.findById(task.requestId);
      if (ticket) {
        let ticketStatus = ticket.status;
        if (updates.status === 'done') ticketStatus = 'COMPLETED';
        else if (updates.status === 'in_progress') ticketStatus = 'IN_PROGRESS';
        ...
        if (ticketStatus !== ticket.status) {
          ...
          ticket.activities.push({
            action: 'STATUS_CHANGED',
            actorId: userId,
            details: { from: oldTicketStatus, to: ticketStatus, note: 'Automatically updated from task status change' }
          });
  ...
  await syncProjectAndTeamStats({ companyId, workspaceId, projectId: task.projectId });
...
export async function getOverdueTasks({ tenantId, workspaceId, userId, role, userType, clientId }) {
  const { Task } = await getTenantModels(tenantId);
  const now = new Date();

  const allowedProjects = await getAccessibleProjectIds({ tenantId, workspaceId, userId, role, userType, clientId });
